...
for i = [1:4]  
    for x = [1:N]
        [PL(i, x), PLv(i, x), APD(i, x), APDv(i, x), ~, ~, ~] = Simulator4(lambda, C, f, P, n(i));
    end
...
title(t, "APD and PL for data and VoIP with diferent number of VoIP flows(n), considering Priority Queuing(Voip > data)");
